...
<body>
	...
	<nav class="navbar navbar-light navbar-expand-sm">
	  ...
	  <div class="collapse navbar-collapse" id="navbarsExample03">
		...
		if(isset($_SESSION['loggedin'])){
			...
			echo '<li class="nav-item active"><a class="nav-link" id="login" href="/login.php">Login</a></li>';
			echo '<li class="nav-item active"><a class="nav-link" id="register" href="/register.php">Register</a></li>';
			echo '</ul>';
		}
		?>
	</div>
    ...
    <?php
        //code for tracking food ordered
        $checked_count = 0;
        $result = $conn->query("SELECT Max(OrderNum) FROM CUSTOMER_ORDER");
        $row = $result->fetch_assoc();
        $orderNumber = $row['Max(OrderNum)'] + 1;
        $cost = 0;
        $cusID = (int)$_SESSION["CustomerID"];
        $payment = "Cash";
		
		#find the payment option you opted for
        if(!empty($_POST['paymentType'])){
            $payment = "";
            foreach($_POST['paymentType'] as $selected){
                $payment = $selected;
				echo "You have opted to pay in: " . $payment . "<br><br>";
            }
        }
		#find the foods you have opted for
        if(!empty($_POST['food'])) {
            $checked_count = count($_POST['food']);
            echo "You have selected and (ordered) the following ".$checked_count." foods(s): <br/>";
            //selected = id
            foreach($_POST['food'] as $selected) {
                $sql = "select * from MENU_ITEM where ItemID = $selected;";
                $result = $conn->query($sql);
                //Shows what customer ordered
                while($row = $result->fetch_assoc()) {
                    echo $row["Name"]. " - $" . $row["Price"]. "<br>";

                    //rounds price to the nearest int
                    $cost += $row["Price"];
                }
            }

            $insert1 = "INSERT INTO CUSTOMER_ORDER VALUES('$cusID', '$orderNumber', '$cost')";
            if($conn->query($insert1) === true){
                echo "Food Total: $". $cost ."<br>Order#".$orderNumber ."<br>";
            } else{
            ...
            foreach($_POST['food'] as $selected) {
                ...
                //Shows what customer ordered
                while($row = $result->fetch_assoc()) {
                    $itmID = (int)$row["ItemID"];
                    $insert2 = "INSERT INTO ORDER_CONTENTS VALUES('$cusID', '$orderNumber', '$itmID')";
                    if($conn->query($insert2) === true){
                       // echo "Please wait for your order<br>";
                    } else{
                        echo "ERROR: Could not able to execute $sql. " . $conn->error;
                    }
                }
            ...
            echo "You have selected ".$checked_count." food(s): <br/>";	
        }
    ?>

    <?php
    // code for tracking game
		$checked_count = 0;
        if(!empty($_POST['game'])) {
            $checked_count = count($_POST['game']);
            echo "<br>You have selected and (ordered) the following ".$checked_count." game(s): <br/>";

            if($checked_count == 1){
                foreach($_POST['game'] as $selected) {
                    $sql = "select * from GAME where GameID = $selected;";

                    $result = $conn->query($sql);
                    while($row = $result->fetch_assoc()) {
                        if($row["GameType"] == "C"){
                            echo "id: " .$row["GameID"]. " - Name: " . $row["ConsoleType"]. "<br>";
                            $gameID = (int)$row["GameID"];
                            $cusUpdate = "UPDATE CUSTOMER SET GameId='$gameID' WHERE CustomerID=$cusID";
                            $conn->query($cusUpdate);
                        }elseif($row["GameType"] == "B"){
                            echo "id: " .$row["GameID"]. " - Name: " .$row["BoardName"]. "<br>";
                            $gameID = (int)$row["GameID"];
                            $cusUpdate = "UPDATE CUSTOMER SET GameId='$gameID' WHERE CustomerID=$cusID";
                            $conn->query($cusUpdate);
                        }else{
                            echo "id: " .$row["GameID"]. " - Name: Computer #" .$row["ComputerNum"]. "<br>";
                            $gameID = (int)$row["GameID"];
                            $cusUpdate = "UPDATE CUSTOMER SET GameId='$gameID' WHERE CustomerID=$cusID";
                            $conn->query($cusUpdate);
                        }
                ...
                echo "!!!Plase only select one game at a time!!!<br>";
                foreach($_POST['game'] as $selected) {
                    $sql = "select * from GAME where GameID = $selected;";
                    ...
                    while($row = $result->fetch_assoc()) {
                        if($row["GameType"] == "C"):?>
                            <input type="radio" name="one[]" value="<?php echo $row['GameID']?>" >
                            <span><?php echo $row['ConsoleType']; ?></span><br>
            <?php       elseif($row["GameType"] == "B"):?>
                            <input type="radio" name="one[]" value="<?php echo $row['GameID']?>" >
                            <span><?php echo $row['BoardName']; ?></span><br>
            <?php       else: ?>
                            <input type="radio" name="one[]" value="<?php echo $row['GameID']?>" >
                            <span><?php echo "computer: ".$row['ComputerNum']; ?></span><br>
            <?php       endif;
                    }
                }
            }
        }else{
            echo "<br>You have checked out ".$checked_count." game(s): <br>";
        }
		
		$checked_count = 0;
        if(!empty($_POST['booth'])) {
            $checked_count = count($_POST['booth']);
            echo "<br>You have selected the following ".$checked_count." booth: <br/>";
 
            foreach($_POST['booth'] as $selected) {

                $sql = "select * from BOOTH where BoothID = $selected;";

                $result = $conn->query($sql);
                while($row = $result->fetch_assoc()) {
                    echo "Booth id: " . $row["BoothID"]. " - Floor: " . $row["Floor"]. " - Seats: " . $row["Seats"]. "<br><br>";
                    $BoothID = (int)$row["BoothID"];

                    $cusUpdate = "UPDATE CUSTOMER SET BoothId='$BoothID' WHERE CustomerID=$cusID";
                    $conn->query($cusUpdate);
                }
            }
        }else{
            echo "<br> You have selected ".$checked_count." booth(s): <br>";
        }
        
        if(!empty($_POST['hrs'])){
            $hours = (int)$_POST['hrs'];
            echo "Expected stay time :".$hours."<br>";
            $cusUpdate = "UPDATE CUSTOMER SET Hours='$hours' WHERE CustomerID=$cusID;";
            $conn->query($cusUpdate);
        }else{
            echo "<br>You have not entered expected time of stay, we assumed you will be staying for 1 hr<br>";
            echo "You can update your stay time at the front desk<br>";
            $cusUpdate = "UPDATE CUSTOMER SET Hours=1 WHERE CustomerID=$cusID;";
            $conn->query($cusUpdate);
        }

    ?>
         <span style="display: inline;">
					<input type="submit" id="btn" name = "btn_submit" value="Finished">
					<input type="submit" id="btn" name = "btn_submit" value="Place another Order">
		</span>
    </form>
	</div>
