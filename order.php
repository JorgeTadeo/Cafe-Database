...
<body>
	...
	<main>
    <! –– TODO: php Session (get customerID), insert data to db(will need help here) ––>
	</div>
	<!-- changed confrim.php to orderreview.php -->
	<form action=confirm.php method="POST" name="checkoutForm">
	<div class="container-fluid justify-cetner sitewidth bottom">
	<! -- header -->
		<div class="text-center py-5">
			<img class="mb-4" src="logo.png" alt="logo" width="72" height="72">
			<h2>Order form</h2>
			<p class="lead">Select the items you want for your order.</p>
		</div>
	<! -- header -->
		
		<div class="row justify-content-md-center">
		<! -- drinks -->		 
				<div class="col-md color">
					<div class="width bottom">
					<h4 class="mb-3">Drinks</h4>
						<?php
							$sql = "select * from MENU_ITEM where itemType = 'D';";
							$result = $mysqli->query($sql);
							while($food = $result -> fetch_assoc()):
						?>

						<input type="checkbox" name="food[]" value="<?php echo $food['ItemID']?>" >
				...
				<div class="col-md">
					...
					<h4 class="mb-3">Games</h4>
						<?php
                            $sql = "select * from GAME where GameType = 'B';";
                            $result = $mysqli->query($sql);
                            while($game = $result -> fetch_assoc()):
                        ?>

                            <input type="checkbox" name="game[]" value="<?php echo $game['GameID']?>" >
                            <span><?php echo $game['BoardName']; ?></span><br>
                        <?php endwhile; ?>
					</div>
				</div>
				<div class="col-md color">
						<div class="width bottom">
						<h4 class="mb-3">Booths</h4>
						<?php
                        ...
                        ?>
                            ...
                            <input type="checkbox" name="booth[]" value="<?php echo $game['BoothID']?>" >
                            <span><?php echo $game['Seats']," Seat Booth"; ?></span><br>
                        <?php endwhile; ?>
						</div>
				</div>
			</div>
			<div class="row">
				<div class="col-md">
						...
						<div class="bottom" style="width: 260px; margin: auto;">
						<?php
                            $sql = "select * from MENU_ITEM where itemType = 'F';";
                            $result = $mysqli->query($sql);
                            while($food = $result -> fetch_assoc()):
                        ?>
				...
				<div class="col-md color">
						<div class="width bottom">
						<h4 class="mb-3">Consoles</h4>
						<div class="bottom" style="width: 170px; margin: auto;">
						<?php
                            $sql = "select * from GAME where GameType = 'C';";
                            $result = $mysqli->query($sql);
                            while($game = $result -> fetch_assoc()):
                        ?>

                            <input type="checkbox" name="game[]" value="<?php echo $game['GameID']?>" >
                            <span><?php echo $game['ConsoleType']; ?></span><br>
                        <?php endwhile; ?>
						</div>
					</div>
				</div>
				<div class="col-md">
						...
						<h4 class="mb-3">How long is your expected stay?</h4>
						</div>
						<div class="bottom" style="width: 125px; margin: auto;">
					    <label for="hrs">Hours:</label>
                        <input type="number" id="hrs" name="hrs" min="1" max="24" align="center">
						</div>
				</div>
			</div>
			<div class="row justify-content-md-center" style="margin-top:0px;width:130px;margin: auto;">
			    <div class="col col-lg-2">
					<input type="submit" id="btn" value="Checkout">
				</div>
			</div>
		</div>
		</form>
